...
public class TriangleTest2 {
    private Triangle2 triangle = new Triangle2();

    // 3.1 一般边界值方法测试用例
    @Test
    void testGeneralBoundaryValues() {
        // 边界值测试用例
        Assertions.assertEquals("非三角形", triangle.classify(10, 15, 70));
        Assertions.assertEquals("非三角形", triangle.classify(21, 60, 90));

        // 一般值测试用例
        Assertions.assertEquals("等腰三角形", triangle.classify(11, 11, 5));
    }
    ...
    void testRobustBoundaryValues() {
        // 边界值测试用例
        Assertions.assertEquals("输入错误", triangle.classify(-1, 1, 7));
        Assertions.assertEquals("输入错误", triangle.classify(-2, -1, 20));
        Assertions.assertEquals("输入错误", triangle.classify(1, -2, 100));
        Assertions.assertEquals("输入错误", triangle.classify(2, 100, -10));

        // 一般值测试用例
        Assertions.assertEquals("等腰三角形", triangle.classify(8, 7, 7));

        // 无效和异常值测试用例
        Assertions.assertEquals("非三角形", triangle.classify(10, 17, 2));
        Assertions.assertEquals("非三角形", triangle.classify(1, 7, 3));
    }

    // 3.3 最坏情况一般边界值分析方法测试用例
    @Test
    void testWorstCaseGeneralBoundaryValues() {
        Assertions.assertEquals("非三角形", triangle.classify(10, 1, 30));
